...
<div class="modal-body">
    ...
    <table class="specsTrim">
        <tr class="row">
            <td class="col-6"><strong>Generation: </strong></td>
            <td class="col-6">{{ $trim->getYearBegin() . '-' . $trim->getYearEnd() }}</td>
        </tr>
        <tr class="row">
            <td colspan="2">&nbsp;</td>
        </tr>
        <tr class="row">
            <td class="col-6"></td>
            <td class="col-6">&nbsp;<strong>Body type: </strong></td>
        </tr>
        <tr class="row">
            <td class="col-6"><img src="{{ fileUrl($trim->getFrameworkImage()) }}" alt="framework"></td>
            <td class="col-6">&nbsp;{{ $trim->getFramework() ?? 'N/A' }}</td>
        </tr>
        <tr class="row">
            <td class="col-6"></td>
            <td class="col-6">&nbsp;<strong>Fuel: </strong></td>
        </tr>
        <tr class="row">
            <td class="col-6">
                ...
                @endif</td>
            <td class="col-6">&nbsp;{{ $trim->getFuel() ?? 'N/A' }}</td>
        </tr>
        <tr class="row">
            <td colspan="2">&nbsp;</td>
        </tr>
        <tr class="row {{ $specsChoice['number_of_doors']['show'] === true ? '' : 'collapseSpecs' }}">
            <td class="col-6">{{ $specsChoice['number_of_doors']['display'] }}</td>
            <td class="col-6">&nbsp;{{ $trim->getNumberOfDoors() ?? 'N/A' }}</td>
        </tr>
        <tr class="row {{ $specsChoice['number_of_seats']['show'] === true ? '' : 'collapseSpecs' }}">
            <td class="col-6">{{ $specsChoice['number_of_seats']['display'] }}</td>
            <td class="col-6">&nbsp;{{ $trim->getNumberOfSeats() ?? 'N/A' }}</td>
        </tr>
        <tr class="row {{ $specsChoice['number_of_gears']['show'] === true ? '' : 'collapseSpecs' }}">
            <td class="col-6">{{ $specsChoice['number_of_gears']['display'] }}</td>
            <td class="col-6">&nbsp;{{ $trim->getNumberOfGears() ?? 'N/A' }}</td>
        </tr>
        <tr class="row {{ $specsChoice['gearbox_type']['show'] === true ? '' : 'collapseSpecs' }}">
            <td class="col-6">{{ $specsChoice['gearbox_type']['display'] }}</td>
            <td class="col-6">&nbsp;{{ $trim->getTransmission() ?? 'N/A' }}</td>
        </tr>
        <tr class="row {{ $specsRange['max_trunk_capacity']['show'] === true ? '' : 'collapseSpecs' }}">
            <td class="col-6">{{ $specsRange['max_trunk_capacity']['display'] }}</td>
            <td class="col-6">&nbsp;{{ is_null($trim->getMaxTrunkCapacity()) ? 'N/A' : $trim->getMaxTrunkCapacity() .
            ' ' . $specsRange['max_trunk_capacity']['unit'] }}</td>
        </tr>
        <tr class="row {{ $specsRange['engine_capacity']['show'] === true ? '' : 'collapseSpecs' }}">
            <td class="col-6">{{ $specsRange['engine_capacity']['display'] }}</td>
            <td class="col-6">&nbsp;{{ is_null($trim->getEngineCapacity()) ? 'N/A' : $trim->getEngineCapacity() .
            ' ' . $specsRange['engine_capacity']['unit'] }}</td>
        </tr>
        <tr class="row {{ $specsRange['fueltank_capacity']['show'] === true ? '' : 'collapseSpecs' }}">
            <td class="col-6">{{ $specsRange['fueltank_capacity']['display'] }}</td>
            <td class="col-6">&nbsp;{{ is_null($trim->getFueltankCapacity()) ? 'N/A' : $trim->getFueltankCapacity() .
            ' ' . $specsRange['fueltank_capacity']['unit'] }}</td>
        </tr>
        <tr class="row {{ $specsRange['max_speed']['show'] === true ? '' : 'collapseSpecs' }}">
            <td class="col-6">{{ $specsRange['max_speed']['display'] }}</td>
            <td class="col-6">&nbsp;{{ is_null($trim->getMaxSpeed()) ? 'N/A' : $trim->getMaxSpeed() .
            ' ' . $specsRange['max_speed']['unit'] }}</td>
        </tr>
        <tr class="row {{ $specsRange['full_weight']['show'] === true ? '' : 'collapseSpecs' }}">
            <td class="col-6">{{ $specsRange['full_weight']['display'] }}</td>
            <td class="col-6">&nbsp;{{ is_null($trim->getFullWeight()) ? 'N/A' : $trim->getFullWeight() .
            ' ' . $specsRange['full_weight']['unit'] }}</td>
        </tr>
        <tr class="row {{ $specsRange['engine_power']['show'] === true ? '' : 'collapseSpecs' }}">
            <td class="col-6">{{ $specsRange['engine_power']['display'] }}</td>
            <td class="col-6">&nbsp;{{ is_null($trim->getEnginePower()) ? 'N/A' : $trim->getEnginePower() .
            ' ' . $specsRange['engine_power']['unit'] }}</td>
        </tr>
        <tr class="row {{ $specsRange['acceleration']['show'] === true ? '' : 'collapseSpecs' }}">
            <td class="col-6">{{ $specsRange['acceleration']['display'] }}</td>
            <td class="col-6">&nbsp;{{ is_null($trim->getAcceleration()) ? 'N/A' : $trim->getAcceleration() .
            ' ' . $specsRange['acceleration']['unit'] }}</td>
        </tr>
        <tr class="row {{ $specsRange['fuel_consumption']['show'] === true ? '' : 'collapseSpecs' }}">
            <td class="col-6">{{ $specsRange['fuel_consumption']['display'] }}</td>
            <td class="col-6">&nbsp;{{ is_null($trim->getFuelConsumption()) ? 'N/A' : $trim->getFuelConsumption() .
