...
/**
 * This file defines SqliteCreateQuery class.
 *
...
class SqliteCreateQuery{
	...
	private function _rewrite_unique_key($matches){
		...
		$this->index_queries[] = "CREATE UNIQUE INDEX $tbl_name" . "$index_name ON " . $tbl_name .$col_name;
		return '';
	}
	/**
	...
	private function _rewrite_key($matches){
		...
		$tbl_name = $this->table_name;
		$this->index_queries[] = 'CREATE INDEX '. $tbl_name . $index_name . ' ON ' . $tbl_name . $col_name ;
		return '';
	}
	/**
